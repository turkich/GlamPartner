...
// this page adds the categories to backend and frontend
...
class _AddcategorieState extends State<Addcategorie> {
  ...
  Future uploadFile() async {
    ...
    for (var img in _image) {
      ...
      ref = firebase_storage.FirebaseStorage.instance
          .ref()
          .child('les categories/${Path.basename(img.path)}');
      await ref.putFile(img).whenComplete(() async {
        await ref.getDownloadURL().then((value) {
          imgRef.add({'url5': value});
          i++;
        });
      });
    }
  ...
  void initState() {
    super.initState();
    imgRef = FirebaseFirestore.instance.collection('imageURL5');
  }
}